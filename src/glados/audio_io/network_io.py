"""
Network Audio I/O for GLaDOS.

This module provides a network-based audio I/O implementation that streams
audio between a remote client and the GLaDOS server. It implements the same
AudioProtocol interface as SoundDeviceAudioIO, so GLaDOS runs unchanged.
...
class NetworkAudioIO:
    ...
    the exact same interface. All GLaDOS logic (VAD, interruption, queues)
    works unchanged.
