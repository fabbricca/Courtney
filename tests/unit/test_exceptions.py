...
import pytest
from glados.core.exceptions import (
    GladosException,
    LLMConnectionError,
    LLMTimeoutError,
...
def test_glados_exception_base():
    """Test base exception with context."""
    exc = GladosException("test error", context={"key": "value"})
...
def test_exception_inheritance():
    ...
    assert issubclass(LLMConnectionError, GladosException)
    assert issubclass(ComponentInitializationError, GladosException)
    assert issubclass(MemoryPersistenceError, GladosException)
