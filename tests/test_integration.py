"""
Integration tests for GLaDOS Web Interface.
...
class MockGLaDOSServer:
    """Mock GLaDOS TCP server for testing."""
    ...
    async def start(self):
        ...
        print(f"Mock GLaDOS server started on {self.host}:{self.port}")
    ...
    async def stop(self):
        ...
        print("Mock GLaDOS server stopped")

    async def handle_client(self, reader, writer):
        """Handle a client connection."""
        print("Mock GLaDOS: Client connected")
        ...
        try:
            while True:
                ...
                print(f"Mock GLaDOS: Received marker 0x{marker:08X}, length {length}")

                # Handle different message types
                if marker == AUTH_TOKEN_FROM_CLIENT:
                    # Simulate authentication
                    token = data.decode('utf-8')
                    print(f"Mock GLaDOS: Auth token: {token}")
                    ...
                    print(f"Mock GLaDOS: Text message: {message}")
            ...
            print("Mock GLaDOS: Connection cancelled")
        except Exception as e:
            print(f"Mock GLaDOS: Error: {e}")
        finally:
            writer.close()
            await writer.wait_closed()
            self.clients.remove((reader, writer))
            print("Mock GLaDOS: Client disconnected")


@pytest.fixture
async def mock_glados_server():
    """Fixture for mock GLaDOS server."""
    server = MockGLaDOSServer()
    await server.start()
    yield server
    await server.stop()
...
@pytest.mark.asyncio
async def test_authentication_flow(mock_glados_server):
...
@pytest.mark.asyncio
async def test_text_message_flow(mock_glados_server):
    ...
    try:
        async with websockets.connect("ws://localhost:8765") as ws:
            ...
            assert 'Echo: Hello GLaDOS' in response_data['message']
...
@pytest.mark.asyncio
async def test_invalid_authentication(mock_glados_server):
...
@pytest.mark.asyncio
async def test_concurrent_connections(mock_glados_server):
